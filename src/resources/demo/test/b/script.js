...
$.fn.booklog = function (json) {
  ...
  for(var i = 0; i < Math.floor(o.count / o.display); i++) {
    ...
    for(var k = 0; k < 4; k++) {
      ...
      var $li      = $('<li/>'),
          ...
          $tip     = $('<div/>').addClass('tool-tip'),
          $tipData = $('<div/>').addClass('book-title')
                                .text(book.title)
                                .after($('<div/>').addClass('book-author').text(book.author));

      $link.html($img).appendTo($li);
      $tip.html($tipData).appendTo($li);
