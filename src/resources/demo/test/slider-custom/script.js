...
/* スタート時は最初のアイテムなので前へを非表示に */
/*
 特定のクラス名を付けて CSS で表示非表示を制御してもいい
 その場合は .addClass('foo') などのようにし、
 CSS で .foo { display: none } や { visibility: hidden } など
 */
$slidePrev.hide();
...
var fnc = {
  ...
  pageNav: function (n) {
             ...
             /*
              ここは下のほうに並んでいるページナビゲーションの管理をする関数だが
              ここで一緒に「前」「次」の状態も合わせて管理することにする
             */
             if(n === 0) {
               $slidePrev.hide();
             } else {
